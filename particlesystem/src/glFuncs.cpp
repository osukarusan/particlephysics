...
void DrawArrow(Vec3d pt, Vec3d dir, double length, double thickness, double arrowthickness){
	Vec3d normDir = norm(dir);
...
Vec3d GetCameraCenterWorld(){
	...
	glGetDoublev( GL_PROJECTION_MATRIX, projmat);
	glGetDoublev( GL_MODELVIEW_MATRIX, modelmat);
	Mat4d InvModelMat=vl_0;
	Mat4d InvProjMat=vl_0;
	for(int i=0; i<4; i++){
		for(int j=0; j<4; j++){
	...
	InvModelMat=trans(InvModelMat*InvProjMat);
	InvModelMat=inv(InvModelMat);
	Vec4d cc = vl_0;
	cc[3]=1;
	cc=InvModelMat*cc;
	if(cc!=0) cc=cc/cc[3];
	return Vec3d(cc[0], cc[1], cc[2]);
}
				...
				GLdouble _focus)
{
	...
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glTranslatef (-_eyedx, -_eyedy, 0.0);
	glMatrixMode(GL_PROJECTION);
}
