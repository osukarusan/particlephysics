...
void Trackball::GetCurrentRotation(Vec4d & returnQuat){
...
void Trackball::Draw(int winWidth, int winHeight)
{
	// draw the axes
	glPushMatrix();
	if(mNonCenter) glTranslatef(mCenter[0], mCenter[1], mCenter[2]);
    ...
    glDisable( GL_LIGHTING );
	glLineWidth(2.0);
	if(mNonCenter) glTranslatef(mStartPix[0], mStartPix[1], 0);
